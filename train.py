...
TRAIN_FILES = provider.getDataFiles( \
    os.path.join(BASE_DIR, 'data/modelnet40_ply_hdf5_2048/MTwMisc/trainWithMisc.txt'))
TEST_FILES = provider.getDataFiles(\
    os.path.join(BASE_DIR, 'data/modelnet40_ply_hdf5_2048/MTwMisc/valWithMisc.txt'))
...
def train():
 ...
 #           print("model1" )
            # Get model and loss
  #(kan behovas laggas till architecture)
            pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, bn_decay=bn_decay)

  # f2.write(str(architecture))
...
#                total_correct_class[l] += (pred_val[i-start_idx] == l)
    ...
    mLoss=(loss_sum / float(total_seen))
    acc = (total_correct / float(total_seen))
    log_string('eval mean loss: %f' % mLoss)      #  (loss_sum / float(total_seen)))
    log_string('eval accuracy: %f'% acc)          #  (total_correct / float(total_seen)))
    timeStop = datetime.now()
    executeTime = timeStop - timeStart
#    log_string('eval avg class acc: %f' % (np.mean(np.array(total_correct_class)/np.array(total_seen_class,dtype=np.float))))
#    TO PLOT THE GRAPHS

#    f.write(str(acc) + '\n')
#    f2.write(str(acc) + '\n')
#    f2.write(str(executeTime)+'\n')
